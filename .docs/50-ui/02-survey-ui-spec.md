...
아동 기질 검사 완료 직후 **아이 기질 리포트를 즉시 무료로 공개**하여, 사용자가 결과에서 느끼는 감동("와!")을 부모 기질 검사 완주의 강력한 동기로 활용합니다.
사용자가 50문항을 모두 완주해야만 결과를 볼 수 있는 구조의 높은 이탈률을 방지하고, 부모 기질 검사 완주율을 높입니다.

```mermaid
graph LR
    A[홈/인트로] --> B(아동 기질 검사 20문항)
    B --> C[아이 기질 리포트 - 무료 즉시 공개]
    C --> D["CTA: 부모님 기질까지 추가하면 더 정밀해요 → 계속하기"]
    D --> E(부모 기질 검사 20문항)
    E --> F{브릿지: 나 성향 완료}
    F --> G(양육 스타일 검사 10문항)
    G --> H[결과 분석 로딩]
...
### 전환 포인트 (C → D 단계)
- **아이 기질 리포트**: 아이 기질 유형, 특성 설명, 아이 전용 AI 심층 리포트까지 무료 공개
- **"계속하기" CTA 카드**: 화면 하단 고정. "부모님 기질을 추가하면 두 기질의 조화 지수(GHI)와 맞춤 양육 솔루션을 받을 수 있습니다" 명시

...
- **보호자의 토양 (The Soil)**: 부모의 TCI + 양육 스타일. 씨앗이 자라나는 기반.
...
| :--- | :--- | :--- |
| **순한 기질 (Easy)** | **둥근 씨앗** | 어디서든 유연하게 굴러가 조화를 이룰 준비가 된 부드러운 본성 |
| **까다로운 기질 (Difficult)** | **뾰족 씨앗** | 자기만의 경계와 결이 선명하여 세심한 조율을 통해 빛나는 개성 |
| **느린 기질 (Slow)** | **단단한 씨앗** | 발아까지 긴 호흡이 필요하지만 그만큼 내실이 굳건하게 채워진 잠재력 |

### 6.3 발견된 새싹의 종류 (아이 TCI)
| TCI 조합 | 메타포 | 새싹의 특징 |
| :--- | :--- | :--- |
| **NS High** | **에너지 넘치는 탐험 새싹** | 새로운 영역을 탐색하며 정원에 역동적인 생명력을 주는 새싹 |
| **HA High** | **섬세한 아티스트 새싹** | 주변을 깊이 있게 관찰하며 자기만의 결을 만들어가는 새싹 |
| **RD High** | **다정한 분위기 메이커 새싹** | 다른 생명체와 교감하며 정원 전체에 온기를 채우는 새싹 |
| **P High** | **단단한 노력가 새싹** | 어떤 환경에서도 포기하지 않고 묵묵히 제 자리를 지키는 새싹 |

### 6.4 정원 상호작용 분석 (Harmony Dynamics)
부모와 아이의 TCI 점수 차이(Delta)가장 큰 항목을 추출하여 관계의 역동성을 분석한다.

| 차이 항목 | 분석 명칭 | 관계의 역동성 메시지 (예시) |
| :--- | :--- | :--- |
| **NS 차이** | **에너지의 속도** | 아이의 질주하려는 에너지와 보호자의 신중한 제동이 만나는 지점 |
| **HA 차이** | **안전의 거리** | 보호자의 보호 본능과 아이의 과감한 도전 사이의 심리적 거리 |
| **RD 차이** | **마음의 온도** | 감정 표현의 방식과 정서적 연결을 제안하는 온도의 문법 차이 |
| **P 차이** | **성취의 호흡** | 목표를 향해 달려가는 호흡과 몰입을 지속하는 리듬의 차이 |
...
1.  **새싹의 발견**: "내 아이는 이런 아이구나"라는 정체성 부여 (감성적 임팩트).
2.  **관계의 정의**: "나(토양)와 아이(씨앗)가 만나 이런 상호작용을 하는구나"라는 인과관계 설명.
3.  **조화의 지점**: 갈등의 원인을 '기질적 차이'로 객관화하여 부모의 심리적 부채감 해소.

### 7.2 유료 버전: "처방과 변화" (Action & Transformation)
구체적인 행동 지침(Actionable Insight)을 제공하여 실제 변화를 이끌어낸다.
1.  **신호 통역**: 아이의 문제 행동 뒤에 숨겨진 기질적 욕구를 번역.
2.  **마법의 한마디**: 부모의 에너지를 최소화하면서 효과를 극대화하는 스크립트.
3.  **미래 비전**: 현재의 새싹이 건강하게 자랐을 때의 구체적인 꽃(Vision) 제시.

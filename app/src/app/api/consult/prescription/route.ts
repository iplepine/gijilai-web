...
5. **실천 항목 (actionItems)**: 정확히 3개의 실천 항목을 제안하세요. 양육자가 이 중에서 하나만 골라 실천합니다. 각 항목은 **If-Then 형식(~할 때, ~하기)**으로 작성하세요.
   - title: **If-Then 형식**으로 작성. "~할 때, ~하기" 구조. (예: "어린이집 현관에서 울 때, 10초 안아주기", "저녁 밥 안 먹겠다 할 때, 선택지 2개 주기"). 30초 이내에 실행 가능한 크기여야 합니다.
   - trigger: If 부분. 이 실천을 할 **구체적 상황/순간** (예: "아이가 어린이집 앞에서 울 때", "잠자리에 누웠을 때"). 일상 루틴에 앵커링할 수 있는 시점이어야 합니다.
   - action: Then 부분. 그 순간에 할 **구체적 행동** (예: "현관에서 10초 안아주며 '엄마도 보고 싶을 거야' 말하기"). 30초 이내 수행 가능.
   ...
   **실천 항목 품질 기준 (반드시 준수)**:
   ...
   - **If-Then 필수**: title은 반드시 "~할 때, ~하기" 구조여야 합니다. trigger(상황)와 action(행동)이 명확해야 합니다.
   ...
   - 나쁜 예: "하루 한 번 아이 감정 공감해주기" → 좋은 예: "어린이집 가기 싫다고 울 때, 현관에서 10초 안아주며 '엄마도 보고 싶을 거야' 말하기"${isFollowUp ? `

...
- 상담 고민 입력, 문진 주관식 답변, 실천 종합 회고처럼 긴 자유 텍스트를 받는 영역에는 음성 입력 버튼을 제공한다.
- 음성 인식은 브라우저 내장 Web Speech API를 우선 사용하며, 미지원 브라우저에서는 버튼을 비활성화하고 기존 키보드 입력은 그대로 유지한다.

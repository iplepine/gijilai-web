...
### 결제
- 구독제: **월 12,000원**
- 연동: 포트원 V2 (NHN KCP + KG 이니시스). 토스페이/네이버페이는 심사 거부로 UI 미노출.
- KG 이니시스 카드 빌링키 발급은 결제창 호출 시 구매자 이름/휴대폰 번호/이메일을 전달해야 한다. 휴대폰 번호는 KG 이니시스 선택 후 구독 버튼을 누른 시점에 다이얼로그로 입력받고 앱 DB에는 저장하지 않는다.

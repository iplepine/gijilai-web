...
- **결정**: 실천 탭에 변화 요약 패널과 알림 설정 진입점을 추가하고, 알림 설정 페이지에서 실천 리마인더 사용 여부와 시간을 저장한다. 웹 브라우저에서는 `localStorage`에만 저장하고, Flutter 앱에서는 WebView `ReminderBridge`를 통해 네이티브 로컬 알림을 매일 반복 예약한다.
